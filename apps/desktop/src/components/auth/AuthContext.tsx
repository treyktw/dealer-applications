...
async function getStoredToken(): Promise<string | null> {
  try {
    ...
    const token = await invoke<string | null>("retrieve_secure", {
      key: STORAGE_KEY,
    });

    if (token) {
      addAuthLog(
        `✅ Token found in keyring (${token.length} chars)`,
        "success"
      );
    } else {
...
async function storeToken(token: string): Promise<void> {
  try {
    ...
    const verify = await invoke<string | null>("retrieve_secure", {
      key: STORAGE_KEY,
    });

    if (!verify) {
      addAuthLog(
        "❌ Verification failed: Token not found after storage",
        "error"
      );
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    addAuthLog(
      "🔧 Convex URL: " + (import.meta.env.VITE_CONVEX_URL || "NOT SET"),
      "info"
    );
  }, []);
  ...
  const { data: sessionData, isLoading: sessionLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      addAuthLog(
        `🔍 Token preview: ${storedToken.substring(0, 20)}...`,
        "info"
      );
      ...
      try {
        ...
        if (result) {
          ...
          addAuthLog(
            `   Dealership: ${result.dealership?.name || "None"}`,
            "success"
          );
          addAuthLog(
  ...
  const handleAuthCallbackMutation = useMutation({
    mutationFn: async ({
      ...
      addAuthLog(
        `📡 Calling Convex with JWT (${clerkJwt.length} chars)`,
        "info"
      );
    ...
    onSuccess: async (data) => {
      ...
      try {
        ...
        addAuthLog("🧪 Testing immediate retrieval...", "info");
        const immediateTest = await getStoredToken();
        if (immediateTest) {
          addAuthLog("✅ Immediate retrieval works!", "success");
        } else {
          addAuthLog("❌ IMMEDIATE RETRIEVAL FAILED!", "error");
        }
      } catch (error) {
  ...
  const handleAuthCallback = useCallback(
    async (event: Event) => {
      ...
      addAuthLog(
        `   Stored state: ${storedState?.substring(0, 20)}...`,
        "info"
      );
